...
// tercera funcion fotos lab service

document.addEventListener("DOMContentLoaded", function () {
    const accessKey = '44Yh3j04VI0h73noLdqOWPkEjsTRqP17zwhwsVNc7W4';
    
    function addImage(photo, cardIndex) {
        const card = document.getElementsByClassName('lab')[cardIndex];
    ...
    // Obtener fotos aleatorias de tu colección de Unsplash
    fetch('https://api.unsplash.com/collections/T74shgPcduE/photos?client_id=' + accessKey + '&order_by=')
    .then(response => {
        if (!response.ok) {
            throw new Error(`Error HTTP! Estado: ${response.status}`);
        }
